...
class TreetrackerModelTrees extends DSCModel 
{
	private $connection;
	private $db;
	...
	function getTable($name='Config', $prefix='TreetrackerTable', $options = array())
	{
	...
	public function __construct() {
		...
		$this->user = '';
		$this->pass = '';
		$this->dbname = 'foodbuntu';
		$this->collection = 'trees';
		$this->persist = TRUE;
		$this->persist_key = 'ci_mongo_persist';
	...
	protected function _buildQueryWhere()
    {
		...
		$filter_latitude   = $this->getState('filter_latitude');
		$filter_longitude   = $this->getState('filter_longitude');
        ...
        {
        	$this->where(array('enabled' => 1));
   
        }
        
        if (strlen($filter_latitude) && strlen($filter_longitude) && strlen($filter_radius) )
        {
...
/**
	...
	public function where_init($param) {
	...
	public function getList() {
		$this->_buildQueryWhere();
	 	...
	 	$results = array();
	 	$documents = $this->db->{$this->collection}->find($this->wheres, $this->selects)->limit((int) $this->limit)->skip((int) $this->offset)->sort($this->sorts);
	 ...
	 public function getItem() {
		$this->_buildQueryWhere();
	 	...
	 	$results = array();
	 	$documents = $this->db->{$this->collection}->findOne($this->wheres, $this->selects);
